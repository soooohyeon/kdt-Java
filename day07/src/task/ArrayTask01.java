...
//		2) 출력 메세지 및 for문 사용해서 설정할 비밀번호 입력받고 정답 배열에 담기
//		3) 사물함 번호 입력받을 출력메세지 및 입력받고 변수에 담아서 비교 후 틀리다면 종료, 맞다면 비밀번호 입력받기
//		4) 출력 메세지 및 for문 사용해서 비밀번호 입력받고 비교 배열에 담기
//		5) for (int i = 0; i < 배열길이; i++) {
//		6) 	if (정답배열명[i] != 입력배열명[i]) { 정답 배열과 비교 배열 차례대로 비교하다 하나라도 틀리다면
//		7)		비밀번호 일치여부 false; }
//		8) 	if (비밀번호 일치여부 false 일 때) {
//		9)		메세지(틀렸습니다) 출력 후 반복문 즉시 종료;
//		10)	} else if (비밀번호 마지막 칸 값까지 일치(i == 5)하고 비밀번호 일치여부도 true일 때) {
//		11) 	메세지(일치합니다) 출력 후 반복문 즉시 종료; } }
//		12) 버퍼 비우기
